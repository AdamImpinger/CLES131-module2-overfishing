...
The unit of obersavation in the 'stock' table is funnily enough stock, with each unit describing a different stock of fish in specific area. These groupings of stock fish are further described by their geographic region, country, and the state of each the stock (current, depreciated).
...
-   What is the unit of observation in the 'assessment' table? How is this related to the unit of observation in the 'stock' table?
    ...
    The unit of observation for the assessment table is also stocks, but in this table as examined over extended time periods. Assessment contextualizes each assessment of different stocks, providing context for when the "state" stat was most recently marked and illuminating the methodology of analysis Some stocks are assessed more than once so they would have more that one assessment data but still only the most recent in stock.

-   It seems like 'stockid' and 'assessyear' might uniquely identify each assessment. Is this the case? If not, why not?

No, there are repeats of assessyears, meaning it can't be used as a unique identifier, stockid however creates unique identifiers for each assessment, tying each examined stock to a region and specific source
